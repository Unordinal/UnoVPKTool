...
namespace XCompression
{
    public sealed class CompressionContext : BaseContext
    {
        ...
        public CompressionContext() : this(Constants.DefaultWindowSize, Constants.DefaultChunkSize) { }

        public CompressionContext(uint windowSize) : this(windowSize, Constants.DefaultChunkSize) { }
        ...
        private CompressionContext(uint windowSize, uint chunkSize)
        {
            ...
            var result = CreateCompressionContext(1, settings, 1, out _Handle);
        ...
        public ErrorCode Compress(byte[] inputBytes, int inputOffset, ref int inputCount, byte[] outputBytes, int outputOffset, ref int outputCount)
        {
            if (inputBytes == null)
            {
            ...
            var result = Compress(_Handle, outputHandle.AddrOfPinnedObject() + outputOffset, ref outputCount, inputHandle.AddrOfPinnedObject() + inputOffset, ref inputCount);
            if (result != ErrorCode.None)
            {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            {
                DestroyCompressionContext(_Handle);
                _Handle = IntPtr.Zero;
            }
