...
namespace XCompression
{
    public sealed class DecompressionContext : BaseContext
    {
        ...
        public DecompressionContext() : this(Constants.DefaultWindowSize, Constants.DefaultChunkSize) { }

        public DecompressionContext(uint windowSize) : this(windowSize, Constants.DefaultChunkSize) { }
        ...
        public ErrorCode Decompress(byte[] inputBytes, int inputOffset, ref int inputCount, byte[] outputBytes, int outputOffset, ref int outputCount)
        {
            if (inputBytes == null)
            {
            ...
            var result = Decompress(_Handle, outputHandle.AddrOfPinnedObject() + outputOffset, ref outputCount, inputHandle.AddrOfPinnedObject() + inputOffset, ref inputCount);
            inputHandle.Free();
            outputHandle.Free();
            return result;
        }
